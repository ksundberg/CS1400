...
namespace lionheart{
  ...
  class mathew_warenski : public Player
  {
  public:
	mathew_warenski(): enemyWaiting(true),id(0){	}
  ...
  private:
	...
	const int ALLY_WEST_WALL = 8;
	  ...
	  lionheart::Direction setdefdir(lionheart::Unit const& u, lionheart::SituationReport);
  ...
  Location getSmartChargePosition(Unit const & u, SituationReport report, Plan p);

  //hide king in center behind wall with infantry sheilds. hide kights, only attack if from behind
  Location hideAndStrike(Unit const & u, SituationReport report, Plan p);

  //assign movements or attacks accordingly
  lionheart::Action playSmartCharge(Unit const & u, SituationReport report, Plan p);
  lionheart::Action playInFortDefend(Unit const & u, SituationReport report, Plan p);
