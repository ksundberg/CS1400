...
namespace
{
    // globals
    auto id = 0;
    bool west = true; // what side of the game board am I playing on
    int numberofturns = 0;
    auto whattodo = 0;
    lionheart::Placement ekingloc;
    lionheart::Placement akingloc;
    lionheart::Placement defloc;
...
// placement function
lionheart::Placement lionheart::EmilyJowers::placeUnit(UnitType,
                                                        StartBox const& box,
...
lionheart::Action lionheart::EmilyJowers::recommendAction(
        Unit const& u, SituationReport report, Plan p)
{
  ...
  if ((numberofturns / 30) <
      60) // this will stall in defense for half the game then attack
  {
    if (p.hasAttack()) return p.attackEnemy();
    if (enemyinthebox(report))
    {
...
lionheart::Blazon lionheart::EmilyJowers::getBlazon()
{
  return {Color::OR,
          Color::ARGENT,
          Style::SALTIRE,
