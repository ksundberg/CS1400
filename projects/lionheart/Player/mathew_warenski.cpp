#include "mathew_warenski.hpp"


  // sets the defensive direction for each piece
  lionheart::Direction lionheart::mathew_warenski::setdefdir(lionheart::Unit const& u, lionheart::SituationReport)
  {
    switch (u.getId())
    {
    case 1:
...
lionheart::Placement lionheart::mathew_warenski::placeUnit(UnitType,
                                              StartBox const &box,
...
if (facingEast)
  ...
  lionheart::Action
lionheart::mathew_warenski::recommendAction(Unit const& u, SituationReport report, Plan p){
...
lionheart::Blazon lionheart::mathew_warenski::getBlazon()
{
  ...
  return { Color::OR, Color::VERT, Style::CROSS, "MathewWarenski" };
}


int lionheart::mathew_warenski::flipIfWest(int x, int w){
	if(facingEast) return x; // no adjust necessary
	int loc =w - x;
	return loc;
}

lionheart::Action lionheart::mathew_warenski::playInFortDefend(Unit const & u, SituationReport report, Plan p){
	auto dir = setdefdir(u,report);
	if(dir != u.getFacing()) return turn(dir);
	if(p.hasAttack()){
...
lionheart::Action lionheart::mathew_warenski::playSmartCharge(Unit const & u, SituationReport report, Plan p){
...
bool lionheart::mathew_warenski::enemyInInbetween(SituationReport report){
...
lionheart::Action lionheart::mathew_warenski::wait(Plan p){
	if(p.hasAttack()){
		return p.attackEnemy();	
	}		
	return Action();
